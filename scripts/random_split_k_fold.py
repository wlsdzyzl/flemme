...
def main(argv):
    ...
    if len(opts) == 0:
        logger.info('unknow options, usage: random_split_k_fold.py -p <dataset_path> --sub_dirs <sub_dirs=.> --subsub_dirs <subsub_dirs=.> --suffix <suffix=\'\'> -o <output_dir=.> -k <kfold=5> --method <method=copy>')
        sys.exit()
    for opt, arg in opts:
        if opt in ('-h', '--help'):
            logger.info('usage: random_split_k_fold.py -p <dataset_path> --sub_dirs <sub_dirs=.> --subsub_dirs <subsub_dirs=.> --suffix <suffix=.> -o <output_dir=.> -k <kfold=5> --method <method=copy>')
            sys.exit()
