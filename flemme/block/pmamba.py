...
def get_psmamba_block(name, **kwargs):
    ...
    if name == 'pmamba':
        ...
        return partial(PointScanMambaNonFFNBlock, **kwargs)
    elif name == 'pmamba2_non_ffn':
        return partial(PointScanMamba2NonFFNBlock, **kwargs)
...
class PointMambaNonFFNBlock(NormBlock):
    def __init__(self, in_channel, out_channel = None, 
        time_channel = 0, state_channel = 64, 
        conv_kernel_size = 4, inner_factor = 2,  
        head_channel = 64,
...
def scan(xyz_features, sorted_index_list):
...
def scan_proj(new_feature, sorted_index_list):
    # B K C L 
    res = []
...
class PointScanMambaBaseBlock(nn.Module):
    ...
    def mamba(self, x, sorted_index_list):
        ...
        xs = scan(x, sorted_index_list)
        out_y = self.ssm(xs)
        y = scan_proj(out_y, sorted_index_list)
...
class PointScanMambaNonFFNBaseBlock(NormBlock):
    def __init__(
        self, in_channel, num_scan, 
        out_channel, state_channel, 
        conv_kernel_size, inner_factor, 
...
class PointScanMambaNonFFNBlock(PointScanMambaNonFFNBaseBlock):
    def __init__(
        self, in_channel, num_scan, out_channel = None, time_channel = 0, 
        state_channel=None, conv_kernel_size=4,
        inner_factor = 2.0,  dt_rank=None, dt_min=0.001, 
...
class PointScanMamba2NonFFNBlock(PointScanMambaNonFFNBaseBlock):
    def __init__(
        self, in_channel, num_scan, out_channel = None, 
        time_channel = 0, state_channel=None, conv_kernel_size=4,
        inner_factor = 2.0,  
