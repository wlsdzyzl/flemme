...
class MBaseBlock(nn.Module):
    def __init__(
        ...
        norm = None, num_group = 0, scan_mode = 'single', flip_scan = False):
        super().__init__()
        ...
        self.norm2 = NormBlock(*(get_norm(norm, self.inner_channel, self.dim, num_group) + (-1,)))
    ...
    def forward(self, x: torch.Tensor):
        ...
        out = self.fc(out) + self.drop_path(self.mlp(out))
        return out
...
class VMambaBlock(MBaseBlock):
    def __init__(
        ...
        dropout=0., drop_path=0.0, conv_bias=True, bias=False, activation = 'silu',
        norm = None, num_group = 0, scan_mode = 'single', flip_scan = False, **kwargs):
        super().__init__(dim = dim, in_channel = in_channel, 
            ...
            num_group = num_group, scan_mode = scan_mode, flip_scan = flip_scan)
        if len(kwargs) > 0:
