...
class PointNetDecoder(nn.Module):
    def __init__(self, point_dim=3, point_num = 2048, 
                in_channel = 256, fc_channels = [256], 
                activation = 'lrelu', dropout = 0.1, 
        ...
        self.pointwise = pointwise
        ## fully connected layer
        fc_channels = [in_channel,] + fc_channels + [point_dim * point_num, ]
        if self.pointwise:
            fc_channels[-1] = self.point_dim
        fc_sequence = [ DenseBlock(fc_channels[i], fc_channels[i+1], 1, 
                                            activation = self.activation, dropout=dropout) for i in range(len(fc_channels) - 1)]
        fc_sequence = fc_sequence + [DenseBlock(fc_channels[-1], fc_channels[-1], 0, activation = None)]
        self.fc = nn.Sequential(*fc_sequence) 
        self.fc_path = fc_channels
        self.fc_path[-1] = point_dim
    ...
    def forward(self, x, t = None):
        x = self.fc(x)
        # project back to the point space
        if not self.pointwise:
            x = x.reshape(-1, self.point_dim, self.point_num).transpose(1, 2)
        return x