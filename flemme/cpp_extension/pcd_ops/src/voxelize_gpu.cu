...
void avg_voxelize(int b, int c, int n, int r, int r2, int r3, const int *coords,
  ...
  grid_stats_kernel<<<b, opt_n_threads(n)>>>(b, n, r, r2, r3, coords, ind,
                                                   cnt);
  avg_voxelize_kernel<<<b, opt_n_threads(n)>>>(b, c, n, r3, ind, cnt,
                                                     feat, out);
  CUDA_CHECK_ERRORS();
}
...
void avg_voxelize_grad(int b, int c, int n, int s, const int *ind,
  ...
  avg_voxelize_grad_kernel<<<b, opt_n_threads(n)>>>(b, c, n, s, ind, cnt,
                                                          grad_y, grad_x);
  CUDA_CHECK_ERRORS();
}